...
//! ## Key Features
//!
//! - **`AtlasDeserialize`**: Automatically implement the `AtlasDeserialize`
...
//!   project.
//! - The macros provided are optimized for use within the atlas framework.
//!
//! ## Contribution
//!
...
///
/// This derive macro creates the `AtlasDeserialize` implementation for a given
...
/// `#[atlas(discriminator)]` attribute, the macro uses this field's value as a
/// discriminator to match and validate data during deserialization.
///
/// # Syntax
///
/// To use this macro, annotate your struct or enum with
/// `#[derive(AtlasDeserialize)]`. Optionally, use the `#[atlas(discriminator
...
///   corresponding `AtlasDeserialize` implementation.
///
/// # Return
///
/// Returns a `TokenStream` representing the generated `AtlasDeserialize`
...
/// # Notes
///
/// - The `#[atlas(discriminator = "0x...")]` attribute is optional. If not
...
#[proc_macro_derive(AtlasDeserialize, attributes(atlas))]
pub fn atlas_deserialize_derive(input_token_stream: TokenStream) -> TokenStream {
...
/// Generates an implementation of the `AtlasDeserialize` trait for a given
/// type.
///
/// This procedural macro automatically derives the `AtlasDeserialize` trait
...
/// and allows for customized deserialization using the `#[atlas]` attribute to
/// specify a unique discriminator for the type.
///
/// # Syntax
///
/// To use this macro, annotate the target type with
/// `#[derive(AtlasDeserialize)]`. Optionally, you can specify a
/// `#[atlas(discriminator = "...")]` attribute to define a custom
...
///   appropriate `AtlasDeserialize` implementation.
///
/// # Return
///
/// Returns a `TokenStream` containing the implementation of the
/// `AtlasDeserialize` trait for the given type. If successful, this enables
...
/// these are the only supported forms for `AtlasDeserialize` derivation.
/// Additionally, an invalid or missing `#[atlas]` attribute may result in a
...
///
/// This function searches through a list of attributes for a `atlas` attribute
/// containing a `discriminator` key in the format `atlas(discriminator =
...
/// `atlas(discriminator = "...")` attribute is not present, the function
/// returns `None`.
///
/// # Syntax
///
...
/// let attrs: Vec<Attribute> = vec![parse_quote!(#[atlas(discriminator = "0x1234")])];
...
/// valid `atlas(discriminator = "...")` attribute is found. If the attribute
/// is not present, or if the value is not a valid hexadecimal string, the
/// function returns `None`.
///
/// # Errors
///
/// If the `atlas(discriminator = "...")` attribute contains an invalid hex
