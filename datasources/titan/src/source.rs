...
impl BitcoinDatasource for TitanBitcoinTxProvider {
    async fn get_transactions(
        &self,
        txids: &[Hash],
    ) -> IndexerResult<std::collections::HashMap<Hash, arch_atlas::bitcoin::Transaction>> {
        // Fetch in parallel
        let futs = txids.iter().map(|h| {
            let txid_str = h.to_string();
            async move {
                let parsed = bitcoin::Txid::from_str(&txid_str)
                    .map_err(|e| arch_atlas::error::Error::Custom(e.to_string()))?;
                ...
                let tx = self
                    ...
                    .await
                    .map_err(|e| arch_atlas::error::Error::Custom(e.to_string()))?;
                Ok::<(Hash, arch_atlas::bitcoin::Transaction), arch_atlas::error::Error>((*h, tx))
            }
        });
