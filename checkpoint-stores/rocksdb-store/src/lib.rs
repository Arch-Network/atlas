...
use arch_atlas::{error::IndexerResult, sync::CheckpointStore};
...
impl RocksCheckpointStore {
    ...
    fn get_u64_cf(&self, cf: &str, key: &str) -> IndexerResult<u64> {
        let handle = self
            .db
            .cf_handle(cf)
            .ok_or_else(|| arch_atlas::error::Error::Custom("Missing stats cf".to_string()))?;
        ...
        match val {
            ...
            Ok(None) => Ok(0),
            Err(e) => Err(arch_atlas::error::Error::Custom(format!(
                "rocksdb get error: {}",
                e
            ))),
        }
    ...
    fn put_u64_cf(&self, cf: &str, key: &str, value: u64) -> IndexerResult<()> {
        ...
        self.db
            ...
            .map_err(|e| arch_atlas::error::Error::Custom(format!("rocksdb put error: {}", e)))?;
        Ok(())
    }
